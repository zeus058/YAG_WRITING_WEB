...
- [3. Architectural Design](#3-architectural-design)
  - [3.1. Architecture Diagram](#31-architecture-diagram)
    ...
    - [3.1.3. Special Architectural Aspects](#313-special-architectural-aspects)
...
- [5. User Interface and User Experience Design](#5-user-interface-and-user-experience-design)
  ...
  - [5.3. The Other UI Design](#53-the-other-ui-design)
- [6. AI Usage Declaration](#6-ai-usage-declaration)
- [7. Presentation](#7-presentation)
- [8. Reflective Report](#8-reflective-report)
  - [8.1 Most helpful sections](#81-most-helpful-sections)
  - [8.2 Unnecessary/Tedious sections](#82-unnecessarytedious-sections)
...
| 4 | **Khung kết quả tìm kiếm** | Hiển thị truyện phù hợp | Box hiển thị truyện phù hợp với tìm kiếm. |
...
Trong quá trình hoàn thiện tài liệu Design và rà soát lại giao diện frontend của dự án YAG, nhóm có sử dụng AI như một công cụ hỗ trợ kiểm tra, gợi ý cải thiện và tổng hợp nội dung khai báo. Việc sử dụng AI tuân thủ nguyên tắc minh bạch: AI không được dùng để thay thế toàn bộ quá trình thiết kế, không được xem là nguồn học thuật chính thức, và mọi nội dung được đưa vào tài liệu đều đã được nhóm đọc lại, đối chiếu với `Proposal.md`, `Requirement.md`, mã nguồn frontend và yêu cầu của đồ án.

| STT |Công cụ| Thời gian| Prompt được chọn | Mục đích sử dụng | Nội dung AI hỗ trợ tạo ra | Sinh viên/nhóm review và chỉnh sửa |
| :---: |:---:|:---:|   :--- | :--- | :--- | :--- |
| 1 | Codex, GPT-5.5|19/05/2026 00:03| **Rà soát tổng thể frontend theo Requirement/Proposal**<br><br>"Phân tích kỹ lưỡng toàn bộ mã nguồn frontend trong thư mục `frontend` dựa trên yêu cầu `Proposal.md` và `Requirement.md` dự án. Frontend hiện tại đã đáp ứng chính xác những yêu cầu đặt ra chưa? Nếu có điểm nào cần hoàn thiện hãy điều chỉnh code frontend. Chuẩn bị phần frontend hoàn chỉnh để kết nối đến backend, database và để deploy sản phẩm thực tế trên mạng. Kiểm tra sự nhất quán trong thiết kế giữa các trang. Nếu có thể nâng cấp, hay tối ưu giao diện cho web tốt hơn thì hãy điều chỉnh." | Rà soát tổng thể frontend theo tài liệu yêu cầu và thiết kế, đặc biệt là khả năng kết nối backend/database/deployment, phân quyền user và tính nhất quán UI. | AI đề xuất và hỗ trợ chỉnh các thành phần frontend như cấu trúc API/env, mock/runtime interaction, role switch Reader/Author/Admin, sidebar theo từng vai trò, footer có link prototype thật, các trang thông tin dự án và một số cải tiến UI. | Thành viên nhóm không sao chép nguyên trạng đề xuất AI mà tự kiểm tra lại với mục tiêu dự án, chạy thử giao diện, gửi thêm ảnh lỗi, yêu cầu bỏ/chỉnh các phần chưa phù hợp như hiệu ứng ở form đăng nhập, tab theo role, thông tin hồ sơ và footer. |
| 2 | Codex, GPT-5.5|19/05/2026 00:07| **Sửa lỗi Reader Mode và loại bỏ nội dung dư thừa**<br><br>"Trước hết, trang đọc truyện đang bị lỗi bố cục cũng như khi chuyển đổi mode màu thì các chữ cũng bị lỗi màu/chìm vào nền, hãy điều chỉnh lại tất cả các lỗi trên trang đọc truyện. Sau đó, phân tích kỹ lại tất cả các trang của dự án, nếu có thông tin nào bị dư thừa với mục đích của trang đó thì hãy loại bỏ và điều chỉnh lại bố cục trang đó cho thích hợp và đẹp." | Kiểm tra sâu trang Reader Mode và rà soát lại nội dung dư thừa trên các trang sau khi frontend đã có bản gần hoàn chỉnh. | AI hỗ trợ sửa Reader Mode: bỏ footer chen vào vùng đọc, thêm cơ chế đổi nền tối/rộng hơn, sửa màu chữ/badge/panel ở dark mode, cải thiện bố cục topbar và toolbar. AI cũng chỉ ra các nội dung lệch vai trò như hồ sơ/thông báo có yếu tố tác giả trong màn hình dùng chung. | Thành viên nhóm trực tiếp review giao diện sau khi chỉnh, xác nhận các lỗi màu/bố cục bằng ảnh chụp, yêu cầu tiếp tục tinh chỉnh để trang đọc thân thiện hơn. Các thay đổi cuối cùng được kiểm tra lại bằng `npm run lint`, `npm run build` và mở thử trên trình duyệt local. |
| 3 | Codex, GPT-5.5|23/05/2026 11:12| **Phân tích `Design.md` theo template Design và đối chiếu với Requirement/Proposal**<br><br>"Phân tích file yêu cầu về trnhf bày báo cáo Desgin cho dự án.<br><br>Phân tích file [Design.md](pa/Design.md) hiện tại, kiểm tra xem đã viết haonf chính các yêu cầu được đưa ra không (mục 6 và 7 chưa cần viết).<br><br>các thông tin trong file Design có thể hiện đầy đủ theo yêu cầu không.<br><br>Phần trình bày trogn design cos thống nhất với file [Requirement.md](pa/Requirement.md) và [Proposal.md](pa/Proposal.md) không?" | Rà soát tài liệu Design trước khi chỉnh sửa: đối chiếu với template Design, Requirement và Proposal; xác định phần nào đã đạt, phần nào thiếu, riêng mục 6 và 7 chưa cần đánh giá nội dung. | AI đọc template Design PDF, `Design.md`, `Requirement.md`, `Proposal.md`; sau đó tạo checklist các vấn đề: Member Contribution thiếu %, thiếu evidence; còn placeholder nội bộ; metadata `Written by/Reviewed by` chưa chuẩn; UI/UX thiếu ảnh example cho các màn hình còn lại; Reflective Report chưa trả lời phần unnecessary/tedious; và một số điểm cần đồng bộ giữa các tài liệu. | Nhóm dùng kết quả này như danh sách kiểm tra, tự đối chiếu lại với file gốc và quyết định những mục cần sửa. Các nhận xét không được đưa nguyên trạng vào báo cáo, mà được dùng để lập kế hoạch chỉnh `Design.md`. |
| 5 |Claude Sonet 4.6|18/05/2026 09:30| **Xây dựng tiêu chuẩn thiết kế UI/UX và phân bổ màn hình**<br><br>"Bạn là 1 chuyên gia thiết kế giao diện UX/UI cho Frontend Web. Bạn hãy dựa trên 2 file Proposal và Requirement để hiểu dự án hiện tại đang làm gì? Hình ảnh đính kèm để hiểu các usecase của dự án? Setup để biết được cấu trúc thư mục dự án đã cấu hình như thế nào rồi? DESIGN để biết được bảng màu nhóm sử dụng. Từ đó, dựa trên kinh nghiệm của bạn, hãy đưa ra 1 bộ các tiêu chuẩn khi thiết kế giao diện phù hợp với website như vậy, cho biết cần có bao nhiêu screen, mỗi screen có chức năng gì, bao gồm những usecase nào. Yêu cầu: giao diện phải tối ưu về mặt perform nhưng vẫn không được quá sơ xài, cố gắng cân bằng trade-off giữa giao diện dễ sử dụng cho nhiều lứa tuổi, thân thiện với người dùng, sử dụng các thiết kế thân thuộc, tránh tình trạng thiết kế sai mục đích, cũng như sử dụng bảng màu 1 cách thông minh, bạn có thể dựa trên 1 bộ quy tắc thiết kế UI nổi tiếng nào đó để làm ràng buộc thiết kế." | Xác định bộ tiêu chuẩn thiết kế UI/UX tổng thể cho dự án YAG và phân bổ hợp lý số lượng màn hình theo nhóm actor (Public, Reader, Author, Admin), đảm bảo bao phủ đầy đủ use case trong use case diagram. | AI đề xuất kiến trúc 21 màn hình phân theo 5 nhóm actor; xây dựng bộ Design Token với Semantic Color Rules (phân biệt rõ màu CTA `#C81C30` và màu lỗi `#EF4444`); đặt ra ràng buộc về Typography (thang 6 cỡ chữ), Spacing (base 4px), Loading UX (cấm spinner toàn trang, bắt buộc Skeleton), Interaction States (4 trạng thái), Accessibility (WCAG AA 4.5:1); đặc tả chức năng chi tiết và use case mapping cho từng màn hình S01–S21; đề xuất luồng điều hướng chính và các trade-off thiết kế quan trọng (màu CTA vs lỗi, Reader Mode hide navbar, Author Studio autosave). | Nhóm đối chiếu danh sách 21 màn hình với use case diagram và `Requirement.md`, loại bỏ các màn hình trùng lặp hoặc vượt phạm vi đồ án, quyết định giữ lại 21 screen như đặc tả trong mục 5.1 Screen Diagram. Bộ Design Token và Semantic Color Rules được kiểm tra lại với `DESIGN.md` gốc của nhóm trước khi đưa vào `tailwind.config.ts`. Các tiêu chuẩn Accessibility và Loading UX được tham chiếu từ Nielsen's 10 Heuristics và Material Design 3 để xác nhận tính chính xác. Nhóm tự viết lại phần mô tả theo ngôn ngữ báo cáo, không đưa nguyên bản output AI vào tài liệu. |

![P1](images_design/P1.png)
![P2](images_design/P2.png)
![P3](images_design/P3.png)
![P4](images_design/P4.png)

## 7. Presentation
Link Video: https://youtu.be/-XydVzs6Kmo

## 8. Reflective Report
### 8.1 Most helpful sections
...
### 8.2 Unnecessary/Tedious sections
Không có.
