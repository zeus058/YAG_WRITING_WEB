...
Hệ thống được thiết kế theo **kiến trúc Modular Monolith**. Trong đó, toàn bộ hệ thống được triển khai dưới dạng một khối thống nhất và chia sẻ chung một Cơ sở dữ liệu Quan hệ. Tuy nhiên, logic nghiệp vụ được phân tách nghiêm ngặt thành các Module độc lập (Domain-Driven Design).

Hệ thống được chia thành 4 tầng:

**a. Tầng hiển thị**

Đây là nơi người dùng trực tiếp tương tác với hệ thống. Tầng này không chứa logic xử lý phức tạp mà chỉ nhận lệnh và hiển thị kết quả:

- _**Web Portal cho Tác giả:**_ Giao diện đăng tải, soạn thảo, sử dụng công cụ AI hỗ trợ viết, dashboard quản lý doanh thu cá nhân và thư viện tác phẩm.
- _**Web Portal cho Độc giả:**_ Giao diện đọc truyện, mua gói Membership, diễn đàn thảo luận, và thanh tìm kiếm AI.
...
- _**API Gateway:**_ Nhận request từ Web Portal và định tuyến nó đến đúng dịch vụ xử lý ở tầng dưới - _ví dụ: Request tìm truyện sẽ được đẩy về Dịch vụ tìm kiếm_.
...
Môi trường này đóng vai phục vụ khi sản phẩm đã cơ bản hoàn thiện và có thể chạy 24/7 (trong mức chi phí được hỗ trợ của GCP) trên các thiết bị có kết nối mạng.
| Thành phần | Dịch vụ đề xuất | Vai trò và Đặc điểm kỹ thuật |
| :--- | :--- | :--- |
| Nền tảng máy chủ | **Google Cloud Run** | Sử dụng mô hình Pay-as-you-go kết hợp Credit hỡ trợ. Hệ thống chỉ khởi chạy khi có yêu cầu, giúp tối ưu hóa ngân sách dự án. |
...
- **Vector DB (Pinecone/Milvus):** Cơ sở dữ liệu chuyên dụng tìm kiếm độ tương đồng vector.
...
- **Vector Database (Pinecone / Milvus):** Lưu trữ các vector ngữ nghĩa phục vụ tính năng tìm kiếm thông minh AI Search.
...
Cơ sở dữ liệu quan hệ (PostgreSQL) không thể tìm kiếm theo ngữ nghĩa (Semantic). Do đó, dữ liệu cốt truyện sẽ được đồng bộ sang **Vector Database** (như Pinecone).
...
- **Database & Storage:** Relational Database (PostgreSQL).
...
- **Development Tools:** Git/GitHub, Docke, Visual Studio Code (IDE).
...
- _**Database**_ Sử dụng Cloud SQL for PostgreSQL để lưu trữ toàn bộ dữ liệu của nền tảng bao gồm tài khoản người dùng, gói membership, nội dung truyện, chương, bình luận và dữ liệu tương tác của độc giả.
...
Dự án được thực hiện bởi nhóm 5 thành viên. Dựa trên năng lực và tỷ lệ đóng góp dự kiến, vai trò của từng cá nhân được phân bổ như sau nhằm đảm bảo hệ thống hoàn thiện đúng tiến độ:
...
- Nguyễn Duy Trường (Software Architect & Backend Developer): Thiết kế chi tiết cơ sở dữ liệu (PostgreSQL, Pinecone) và phát triển các API nghiệp vụ lõi như viết truyện, quản lý thành viên.
...
| :---------------------- | :------------------------- | :--------------------------- | :------------------------------------------------------------------------------------------------------------------- |
| **Frontend Hosting**    | Vercel / Firebase Hosting  | $0.00                        | Sử dụng gói Hobby/Free Tier, đáp ứng hoàn toàn băng thông cho giai đoạn phát triển và báo cáo.                       |
| **Backend & API**       | Google Cloud Run           | $0.00                        | Tận dụng tín dụng (Credit) GCP dành cho sinh viên; mô hình Serverless chỉ tính phí khi có request thực tế phát sinh. |
| **Relational Database** | Supabase (PostgreSQL)      | $0.00                        | Cung cấp sẵn cơ sở dữ liệu quan hệ mạnh mẽ kèm gói Free đủ dung lượng lưu trữ nội dung truyện và tài khoản.          |
| **Vector Database**     | Pinecone                   | $0.00                        | Gói Starter miễn phí hỗ trợ lưu trữ vector chuyên dụng cho tính năng AI Search.                                      |
| **In-memory Cache**     | Upstash (Serverless Redis) | $0.00                        | Quản lý Rate Limiting và hàng đợi thông báo mà không tốn phí duy trì server 24/7.                                    |
| **AI Engine**           | Google Gemini API          | $0.00                        | Gói Free Tier (15 RPM / 1 triệu token/phút) đáp ứng đủ nhu cầu kiểm duyệt và gợi ý văn bản cho đồ án.                |
| **Lưu trữ ảnh (Media)** | Cloudinary                 | $0.00                        | Gói miễn phí cung cấp 25 credits/tháng, tối ưu hóa việc phân phối ảnh bìa truyện qua CDN.                            |
| **Tên miền (Domain)**   | Tùy chọn (.tech / .me)     | ~$2.00 - $5.00               | (Tùy chọn) Có thể đăng ký miễn phí 1 năm đầu tiên thông qua GitHub Student Developer Pack.                           |
...
Trong quá trình thực hiện đồ án và xây dựng bản Proposal này, nhóm có sử dụng công cụ Trí tuệ Nhân tạo (AI) với vai trò là công cụ hỗ trợ rà soát logic hệ thống tuân thủ nguyên tắc minh bạch và liêm chính học thuật.

- **Công cụ:** Gemini 3.1.

- **Mục đích sử dụng:** Hỗ trợ phát hiện các lỗi mâu thuẫn logic trong kiến trúc phần mềm do các thành viên ghép bài chưa đồng nhất; gợi ý định dạng trình bày (bảng biểu).

- **Nội dung do AI tạo ra:** Lời khuyên về việc loại bỏ Cloud Firestore để đồng nhất dùng PostgreSQL; đề xuất khắc phục lỗi Real-time WebSocket trên Cloud Run bằng Redis Pub/Sub.
