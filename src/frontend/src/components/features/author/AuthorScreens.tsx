...
export function PublishScreen() {
  return <AppShell activeId="s17"><section className="panel panel-pad stack"><div className="stepper"><div className="step active">1. Thông tin</div><div className="step active">2. Xem trước & cam kết</div></div><div className="grid grid-3"><div className="field"><label>Tên chương</label><input className="input" defaultValue="Tiếng còi cuối mùa" /></div><div className="field"><label>Số thứ tự</label><input className="input" defaultValue="13" /></div><div className="field"><label>Loại chương</label><select className="select"><option>Miễn phí</option><option>Premium</option></select></div></div><div className="layout-2"><div className="publish-preview"><h2 className="section-title">Bản đọc thử</h2><p>Mưa đã ngừng khi An quay lại sân ga. Những ô cửa kính phản chiếu thành phố như một bản thảo chưa kịp sửa.</p></div><div className="stack"><label className="pill"><input type="checkbox" defaultChecked /> Tôi cam kết nội dung tuân thủ chính sách.</label><button className="button button-primary" data-toast="Chương đã được gửi kiểm duyệt."><Icon name="arrow" />Xuất bản</button><button className="button" data-toast="Đã lưu bản nháp.">Lưu nháp</button><ErrorGuide title="Không xuất bản được?" items={["Kiểm tra tên chương, số thứ tự và loại chương đã chọn.", "Bật ô cam kết nội dung trước khi gửi kiểm duyệt.", "Nếu bị cảnh báo bản quyền, hãy thay phần trích dẫn bằng nội dung tự viết."]} /></div></div></section></AppShell>;
}

export function ScheduleScreen() {
  return <AppShell activeId="s18"><section className="layout-right"><main className="panel panel-pad stack"><div className="inline-actions" style={{ justifyContent: "space-between" }}><h2 className="section-title">Tháng 05/2026</h2><button className="button button-primary" data-toast="Đã thêm lịch đăng."><Icon name="calendar" />Thêm lịch</button></div><div className="calendar">{Array.from({ length: 28 }, (_, index) => <div className="calendar-day" key={index}><strong>{index + 1}</strong>{index % 5 === 0 ? <span className="badge badge-amber">Sắp trễ</span> : null}</div>)}</div></main><aside className="stack"><div className="panel panel-pad stack"><h2 className="section-title">Tỷ lệ đúng lịch</h2><div className="donut"><div className="donut-inner">82%</div></div><div className="notice warning"><Icon name="bell" />Chương 14 đến hạn trong 22 giờ.</div><ErrorGuide title="Nguy cơ trễ lịch" items={["Đổi lịch trước 12 giờ để không ảnh hưởng điểm uy tín.", "Nếu chương chưa sẵn sàng, hãy đăng thông báo ngắn cho độc giả."]} /></div></aside></section></AppShell>;
}
