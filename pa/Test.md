...
| Input Data | - JWT của tài khoản author sở hữu chương <br> - `POST /api/v1/chapters/{chapter_id}/publish` body: `{ "publish_at": "2026-06-05T10:00:00Z" }` <br> - Nội dung chương: Văn bản an toàn, không vi phạm chính sách |
...
| Input Data | - JWT của tài khoản author <br> - `POST /api/v1/chapters/{chapter_id}/publish` với nội dung vi phạm: <br> &nbsp; • Nội dung NSFW: "..." <br> &nbsp; • Nội dung bạo lực: "..." <br> &nbsp; • Spam/tự quảng cáo: "..." <br> - Gemini API được mock để trả về `result = "flagged"` hoặc `"rejected"`, có `flagged_categories`, `confidence`, `reason` |
...
| Context | Khi nhân viên push code lên GitHub, hệ thống CI/CD tự động chạy Linter (ESLint, Pylint) và Unit/Integration tests (pytest); nếu fail sẽ chặn merge và yêu cầu fix lỗi |
...
| Test steps | 1. **Chuẩn bị:** Clone repo YAG, tạo feature branch từ main. <br> 2. **Tình huống 1 (Pass):** <br> &nbsp; a. Viết code sạch (tuân thủ linter rules, có test coverage). <br> &nbsp; b. Push lên GitHub. <br> &nbsp; c. Xác nhận GitHub Actions workflow trigger. <br> &nbsp; d. Kiểm tra lint job `conclusion = success`. <br> &nbsp; e. Kiểm tra pytest/build job `conclusion = success`. <br> &nbsp; f. Kiểm tra Pull Request không bị block bởi required checks. <br> 3. **Tình huống 2 (Lint fail):** <br> &nbsp; a. Viết code có lỗi linting (VD: unused import, trailing comma missing). <br> &nbsp; b. Push lên GitHub. <br> &nbsp; c. Kiểm tra GitHub Actions trigger. <br> &nbsp; d. Kiểm tra lint job `conclusion = failure`, log có lỗi cụ thể. <br> &nbsp; e. Kiểm tra Pull Request bị block do required check không pass. <br> &nbsp; f. Kiểm tra "Details" link dẫn đến log lỗi chi tiết. <br> 4. **Tình huống 3 (Pytest fail):** <br> &nbsp; a. Viết code pass linting nhưng có test fail (VD: function logic sai). <br> &nbsp; b. Push lên GitHub. <br> &nbsp; c. Kiểm tra lint job `conclusion = success`. <br> &nbsp; d. Kiểm tra pytest job `conclusion = failure`, log có test failure chi tiết. <br> &nbsp; e. Kiểm tra Pull Request bị block do required check không pass. <br> &nbsp; f. Developer fix code, push lại. <br> &nbsp; g. Kiểm tra workflow chạy lại tự động trên branch mới. |
| Actual Output | <div><strong>Tình huống 1:</strong> Workflow có <code>conclusion = success</code>, PR không bị block bởi required checks.</div><div><strong>Tình huống 2:</strong> Lint job có <code>conclusion = failure</code>, PR bị block và log hiển thị lỗi cụ thể.</div><div><strong>Tình huống 3:</strong> Lint pass nhưng pytest job có <code>conclusion = failure</code>, PR bị block và log hiển thị test failure chi tiết. Developer fix code, push lại, workflow chạy lại.</div> |
| Result | Passed |
