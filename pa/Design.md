...
flowchart TD
    %% ======= KHAI BÁO MÀU SẮC =======
    classDef presentation fill:#eef2ff,stroke:#4338ca,stroke-width:2px,color:#000
    classDef gateway fill:#f0fdfa,stroke:#0d9488,stroke-width:2px,color:#000
    classDef core fill:#f5f3ff,stroke:#6d28d9,stroke-width:2px,color:#000
    classDef data fill:#fefce8,stroke:#b45309,stroke-width:2px,color:#000
    classDef external fill:#fff1f2,stroke:#be123c,stroke-width:2px,color:#000

    %% ===== TẦNG PRESENTATION =====
    subgraph Tier1 ["1. Presentation Tier"]
        direction LR
        C1_1[Web Portal - Độc giả]
        C1_2[Author Studio - Tác giả]
        C1_3[Admin Dashboard]
    end
    class C1_1,C1_2,C1_3 presentation
    style Tier1 fill:#e0e7ff,stroke:#4338ca,stroke-width:2px

    %% ===== TẦNG GATEWAY =====
    subgraph Tier2 ["2. Gateway & Security Tier"]
        direction LR
        C2_1[Nginx API Gateway]
        C2_2[JWT Auth & RBAC]
        C2_3[Rate Limiter & Anti-Bot]
    end
    class C2_1,C2_2,C2_3 gateway
    style Tier2 fill:#ccfbf1,stroke:#0d9488,stroke-width:2px

    %% ===== TẦNG BACKEND =====
    subgraph Tier3 ["3. Core Business Logic Tier (Modular Monolith)"]
        direction LR
        C3_1[Writing & Content]
        C3_2[Community & Interaction]
        C3_3[Membership & Payment]
        C3_4[AI Smart Engine]
        C3_5[Admin, Moderation & Scheduler]
    end
    class C3_1,C3_2,C3_3,C3_4,C3_5 core
    style Tier3 fill:#ede9fe,stroke:#6d28d9,stroke-width:2px

    %% ===== TẦNG DATA =====
    subgraph Tier4 ["4. Data & Infrastructure Tier"]
        direction LR
        C4_1[(PostgreSQL)]
        C4_2[(pgvector)]
        C4_3[(Redis)]
    ...
    class C4_1,C4_2,C4_3,C4_4 data
    style Tier4 fill:#fef3c7,stroke:#b45309,stroke-width:2px

    %% ===== TẦNG EXTERNAL =====
    subgraph Tier5 ["5. External 3rd-Party Services"]
        direction LR
        C5_1[Google Gemini API]
        C5_2[VNPAY Gateway]
        C5_3[Cloudinary CDN]
        C5_4[Google Cloud Storage]
    end
    class C5_1,C5_2,C5_3,C5_4 external
    style Tier5 fill:#ffe4e6,stroke:#be123c,stroke-width:2px

    %% ===== Luồng kết nối (Data Flow) =====
    Tier1 -->|HTTP/HTTPS/WSS| Tier2
    Tier2 -->|Route & Validate| Tier3

    Tier3 -->|Read/Write| Tier4
    Tier3 -->|API Calls| Tier5

    %% Backup connection
    C4_1 -.->|Auto Backup| C5_4
```
...
flowchart TD
    %% ======= KHAI BÁO MÀU SẮC =======
    classDef client fill:#eef2ff,stroke:#4338ca,stroke-width:2px,color:#000
    classDef gateway fill:#f0fdfa,stroke:#0d9488,stroke-width:2px,color:#000
    classDef backend fill:#f5f3ff,stroke:#6d28d9,stroke-width:2px,color:#000
    classDef data fill:#fefce8,stroke:#b45309,stroke-width:2px,color:#000
    classDef ext fill:#fff1f2,stroke:#be123c,stroke-width:2px,color:#000

    %% ===== TẦNG CLIENT =====
    subgraph Client_Layer [" "]
        direction LR
        Web[Web Portal]:::client
        Studio[Author Studio]:::client
        AdminUI[Admin Dashboard]:::client
    end
    style Client_Layer fill:#e0e7ff,stroke:#4338ca,stroke-width:2px

    %% ===== TẦNG GATEWAY =====
    subgraph Gateway_Layer [" "]
        Nginx["API Gateway (Auth, Rate Limit)"]:::gateway
    end
    style Gateway_Layer fill:#ccfbf1,stroke:#0d9488,stroke-width:2px

    %% ===== TẦNG BACKEND =====
    subgraph Backend_Layer [" "]
        direction LR
        Content[Content & Writing]:::backend
        Comm[Community & Forum]:::backend
        AdminSys[Admin & Scheduler]:::backend
        Payment[Membership & Payment]:::backend
        AI[AI Engine & Moderation]:::backend
    end
    style Backend_Layer fill:#ede9fe,stroke:#6d28d9,stroke-width:2px

    %% ===== TẦNG DATA =====
    subgraph Data_Layer [" "]
        direction LR
        RabbitMQ[RabbitMQ Queue]:::data
        Redis[(Redis Cache)]:::data
        Postgres[(PostgreSQL DB)]:::data
        PgVector[(pgvector Extension)]:::data
    end
    style Data_Layer fill:#fef3c7,stroke:#b45309,stroke-width:2px

    %% ===== TẦNG EXTERNAL =====
    subgraph External_Layer [" "]
        direction LR
        Cloudinary[Cloudinary CDN]:::ext
        GCS[Google Cloud Storage]:::ext
        VNPAY[VNPAY Sandbox]:::ext
        Gemini[Google Gemini API]:::ext
    end
    style External_Layer fill:#ffe4e6,stroke:#be123c,stroke-width:2px

    %% ===== LUỒNG GIAO TIẾP =====
    Web -- "HTTPS / WSS" --> Nginx
    Studio -- "HTTPS / WSS" --> Nginx
    AdminUI -- "HTTPS" --> Nginx

    Nginx -- "HTTP / WSS" --> Content
    Nginx -- "HTTP / WSS" --> Comm
    Nginx -- "HTTP Proxy" --> AdminSys
    Nginx -- "HTTP Proxy" --> Payment
    Nginx -- "HTTP Proxy" --> AI

    %% ===== DATA ACCESS & ASYNC =====
    Content == "AMQP" ==> RabbitMQ
    AdminSys == "AMQP" ==> RabbitMQ
    RabbitMQ == "AMQP" ==> AI

    Content -. "TCP Cache" .-> Redis
    Comm -. "TCP Cache" .-> Redis

    Content -- "TCP SQL" --> Postgres
    Comm -- "TCP SQL" --> Postgres
    AdminSys -- "TCP SQL" --> Postgres
    Payment -- "TCP SQL" --> Postgres
    AI -- "TCP SQL" --> Postgres

    AI -- "TCP Vector" --> PgVector
    Content -- "HTTP (AI Suggest)" --> AI

    %% ===== EXTERNAL INTEGRATION =====
    Content -. "API" .-> Cloudinary
    Comm -. "API" .-> Cloudinary
    Web -. "CDN Fetch" .-> Cloudinary
...
---
flowchart TD
    %% ======= KHAI BÁO MÀU SẮC NODE =======
    classDef auth fill:#f3f4f6,stroke:#6b7280,stroke-width:2px,color:#000
    classDef main fill:#eef2ff,stroke:#4338ca,stroke-width:2px,color:#000
    classDef reader fill:#f0fdfa,stroke:#0d9488,stroke-width:2px,color:#000
    classDef payment fill:#fefce8,stroke:#b45309,stroke-width:2px,color:#000
    classDef author fill:#faf5ff,stroke:#9333ea,stroke-width:2px,color:#000
    classDef admin fill:#fff1f2,stroke:#be123c,stroke-width:2px,color:#000

    %% ===== 1. AUTH & ONBOARDING =====
    subgraph G_Auth ["1. Authentication & Onboarding"]
        direction LR
        S01([S01: Landing Page]):::auth
        S02([S02: Đăng nhập / Đăng ký]):::auth
        S03([S03: Khôi phục mật khẩu]):::auth
        S01 --> S02
        S02 -.-> S03
    end
    %% Nền Xám nhạt
    style G_Auth fill:#f9fafb,stroke:#6b7280,stroke-width:2px

    %% ===== 2. MAIN NAV & READER HUB =====
    subgraph G_Main ["2. Main Navigation Hub"]
        direction TB
        S04([S04: Home Feed]):::main

        subgraph G_Main_Sub [" "]
            direction LR
            S05([S05: Khám phá & Tìm kiếm]):::main
            S11([S11: Thư viện cá nhân]):::main
            S08([S08: Diễn đàn]):::main
            S12([S12: Hồ sơ cá nhân]):::main
            S14([S14: Trung tâm thông báo]):::main
        end
        style G_Main_Sub fill:none,stroke:none

        S13([S13: Cài đặt tài khoản]):::main

        S04 --> S05 & S11 & S08 & S12 & S14
        S12 --> S13
    end
    %% Nền Indigo nhạt
    style G_Main fill:#e0e7ff,stroke:#4338ca,stroke-width:2px

    %% ===== 3. READING EXPERIENCE =====
    subgraph G_Reader ["3. Reading Experience"]
        direction LR
        S06([S06: Trang truyện]):::reader
        S07([S07: Reader Mode]):::reader
        S06 --> S07
    end
    %% Nền Teal nhạt
    style G_Reader fill:#ccfbf1,stroke:#0d9488,stroke-width:2px

    %% ===== 4. MEMBERSHIP & PAYMENT =====
    subgraph G_Payment ["4. Payment Flow"]
        direction LR
        S09([S09: Membership]):::payment
        S10([S10: Kết quả thanh toán]):::payment
        S09 -->|Thanh toán VNPAY| S10
    end
    %% Nền Vàng/Cam nhạt
    style G_Payment fill:#fef3c7,stroke:#b45309,stroke-width:2px

    %% ===== 5. AUTHOR WORKSPACE =====
    subgraph G_Author ["5. Author Workspace"]
        direction TB
        S15([S15: Thư viện tác phẩm]):::author
        S16([S16: Author Studio - Editor]):::author
        S17([S17: Xuất bản chương]):::author
        S18([S18: Lịch đăng & Cam kết]):::author

        S15 --> S16 & S18
        S16 --> S17
    end
    %% Nền Tím nhạt
    style G_Author fill:#f3e8ff,stroke:#9333ea,stroke-width:2px

    %% ===== 6. ADMIN CONTROL PANEL =====
    subgraph G_Admin ["6. Admin Control Panel"]
        direction LR
        S19([S19: Admin Dashboard]):::admin
        S20([S20: Kiểm duyệt nội dung]):::admin
        S21([S21: Thống kê & Báo cáo]):::admin

        S19 --> S20 & S21
    end
    %% Nền Đỏ/Hồng nhạt
    style G_Admin fill:#ffe4e6,stroke:#be123c,stroke-width:2px


    %% ==========================================
    %% LUỒNG KẾT NỐI CHÉO GIỮA CÁC PHÂN HỆ
    %% ==========================================

    %% Auth -> Hubs
    S02 ==>|Độc giả / Tác giả| S04
    S02 ==>|Tài khoản Admin| S19

    %% Main -> Reader
    S05 & S11 --> S06

    %% Reader <-> Payment
    S06 -.->|Yêu cầu đọc Chương VIP| S09
    S07 -.->|Hết hạn VIP| S09
    S10 -.->|Quay lại đọc| S06

    %% Main -> Author
    S04 ==>|Chuyển sang chế độ Tác giả| S15

    %% Phản hồi thông báo (Background Processes)
    S17 -.->|Kết quả AI Kiểm duyệt| S14
    S18 -.->|Nhắc nhở / Cảnh báo trễ lịch| S14
    S20 -.->|Kết quả Xử lý / Cảnh báo| S14
```
